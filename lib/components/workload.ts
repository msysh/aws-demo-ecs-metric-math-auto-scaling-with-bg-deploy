...
export class Workload extends Construct implements IWorkload {
  ...
  constructor(scope: Construct, id: string, props: WorkloadProps) {
    ...
    const prodTargetGroup = new elbv2.ApplicationTargetGroup(this, 'TargetGroup1', {
      port: 80,
      protocol: elbv2.ApplicationProtocol.HTTP,
    ...
    const testTargetGroup = new elbv2.ApplicationTargetGroup(this, 'TargetGroup2', {
      port: 80,
      protocol: elbv2.ApplicationProtocol.HTTP,
      protocolVersion: elbv2.ApplicationProtocolVersion.HTTP1,
      targetType: elbv2.TargetType.IP,
    ...
    const repository = new ecr.Repository(this, 'EcrRepository', {
      ...
      removalPolicy: RemovalPolicy.RETAIN, // If DESTROY, error occurs in deleting stack
    });
    ...
    new ecs.CfnClusterCapacityProviderAssociations(scope, 'CapacityProvider', {
      capacityProviders: [
        'FARGATE_SPOT'
      ],
    ...
    const service = new ecs.FargateService(this, 'EcsService', {
      ...
      deploymentController: {
        type: ecs.DeploymentControllerType.CODE_DEPLOY // ECS: rolling update, CODE_DEPLOY: blue/green
      },
      desiredCount: 0,
    ...
    service.attachToApplicationTargetGroup(prodTargetGroup);
    ...
    new cdk.CfnOutput(this, 'Output-TargetGroupName1', {
      description: 'Target Group Name 1',
      value: prodTargetGroup.targetGroupFullName
    });
    new cdk.CfnOutput(this, 'Output-TestListener', {
      description: 'ALB Listener for Test',
      value: testListener.listenerArn,
    });
    new cdk.CfnOutput(this, 'Output-TargetGroupName2', {
      description: 'Target Group Name 2',
      value: testTargetGroup.targetGroupFullName,
    });
    ...
    new cdk.CfnOutput(this, 'Output-EcsServiceLogGroup', {
      description: 'ECS Service LogGroup',
    ...
    new cdk.CfnOutput(this, 'Output-RoleTask', {
      description: 'ECS Task Role Arn',
      value: taskRole.roleArn,
    });
    new cdk.CfnOutput(this, 'Output-RoleTaskExecution', {
    ...
    this.blueTargetGroup = prodTargetGroup;
    this.greenTargetGroup = testTargetGroup;
